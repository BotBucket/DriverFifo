...
void receive_NB(char *P[]){
	...
	char buffer[bufferSize],nbc, options = '1';
...
void send_NB(char *P[]){
        ...
        char buffer[bufferSize], options = '1';
	int length;
        memset(buffer, 0 , sizeof(buffer));
	printf("\n");
	printf("*******************************\n");
...
//		exit(2);
	...
	//		exit(6);
		...
		printf("\n********** ********** ********** ********** **********\n\n");
                printf("Write again ? 1 : Yes \tAnything else : No :\n");
//              while ((getchar()) != '\n');
                options = fgetc(stdin);
        }while( options == '1');
